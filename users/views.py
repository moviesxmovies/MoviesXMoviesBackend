...
@cached_view(
    make_key=lambda req, last_id=None, limit=10: f'suggested_users:{req.user.pk}:{last_id}:{limit}',
    timeout=60 * 15,
)
def suggested_users(request, last_id: int, limit: int) -> JsonResponse:
    ...
    Args:
        request: The authenticated incoming HTTP request.
        last_id (int): The last ID for progressive pagination, injected by ``get_query_params``.
...
# REVIEWS
@extend_schema(
    responses={200: ReviewSerializer.get_progressive_pagination_schema(), 400: None, 404: None},
...
@get_query_params('last_id', 'limit')
@cached_view(
    make_key=lambda req, user, last_id=None, limit=10: f'user_reviews:{user.pk}:{last_id}:{limit}',
    timeout=60 * 5,
)
def user_reviews(request, user: User, last_id: int = None, limit: int = 10) -> JsonResponse:
    ...
    """
    return get_progressive_response(
        user.reviews.all(), ReviewSerializer, request, last_id, limit, '-created_at'
    )


# FRIENDS
def _get_friends_response(request, user: User, last_id: int, limit: int) -> JsonResponse:
    return get_progressive_response(user.get_friends(), UserSerializer, request, last_id, limit)
...
@get_query_params('last_id', 'limit')
@cached_view(
    make_key=lambda req, user, last_id=None, limit=10: f'user_friends:{user.pk}:{last_id}:{limit}',
    timeout=60 * 5,
)
def user_friends(request, user: User, last_id: int = None, limit: int = 10) -> JsonResponse:
    ...
    """
    return _get_friends_response(request, user, last_id, limit)
...
def self_friends(request, last_id: int = None, limit: int = 10) -> JsonResponse:
    ...
    """
    return _get_friends_response(request, request.user, last_id, limit)


@extend_schema(
    responses={
        200: FriendRequestSerializer.get_progressive_pagination_schema(),
        400: None,
        404: None,
    },
