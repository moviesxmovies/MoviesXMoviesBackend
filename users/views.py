...
class VerifyUserSerializer(serializers.Serializer):
    verification_code = serializers.CharField(
        required=True, help_text="Verification code"
    )
...
def verify_user(request, body):
    ...
    return JsonResponse(
        {"error": "Verification code is incorrect"}, status=HTTPStatus.BAD_REQUEST
    )
...
@extend_schema(
    ...
    parameters=[
        ...
        OpenApiParameter(
            name="limit", description="Items per page", required=False, type=int
        ),
    ],
)
@api_view(["GET"])
