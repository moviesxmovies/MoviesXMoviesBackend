...
def test_auth_malformed_token(rf, mock_view_auth_required):
    ...
    assert (
        ...
        == "Token is invalid or have an incorrect padding"
    )
...
def test_send_verification_email_logic(user_factory):
    ...
    expected_html = render_to_string(
        "users/email/verification-email.html", {"user": user}
    )
...
def test_user_signup(auth_client):
    ...
    response = auth_client.post(
        SIGNUP_URL, data=payload, content_type="application/json"
    )
    assert response.status_code == HTTPStatus.OK
...
def test_follow_user(auth_client, user_factory):
    ...
    response = auth_client.post(
        FOLLOW_USER_URL.format(username=user_to_follow.username)
    )
...
def test_unfollow_user(auth_client, user_factory):
    ...
    response = auth_client.delete(
        FOLLOW_USER_URL.format(username=user_to_unfollow.username)
    )
...
def test_edit_user(auth_client):
    ...
    response = auth_client.put(
        SELF_USER_WRAPPER_URL, data=payload, content_type="application/json"
    )
    assert response.status_code == HTTPStatus.OK
